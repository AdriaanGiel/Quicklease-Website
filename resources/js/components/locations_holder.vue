<template>
        <div id="locations_holder" class="locations_holder">
            <ul id="locations" class="clearfix locations">
                <li class="float-left custom-list-item-holder custom-list-item" v-for="(locations, num) in newarray" :key="num">
                    <Location-button :location="location" v-for="location in locations" :key="location.id"></Location-button>
                </li>
            </ul>
...
<script>
    ...
    export default {
        ...
        mounted() {
            School.all().then((res) => {
                this.locations = res.data.schools.map((school) => {
